...
class ILPCostProblemMatrix:
    ...
    def make_objective(self, col, w):
        ...
        # Cost * y -> min
        self.f.iloc[0, column] = list(self.graph.cost_sta.values())
    ...
    def make_equality(self, row, col, y):
        ...
        data_eq = np.zeros([row, len(col)]).astype(int)
    ...
    def create_matrix(self):
        ...
        self._bool_num = (1 + len(y_name))

        self.make_objective(col_name, y_name)
        ...
        y_column = np.where(np.in1d(self.f.columns.values, y_name))
        self.upper_bounds[0, y_column] = 1
